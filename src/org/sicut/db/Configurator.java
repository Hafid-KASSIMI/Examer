...
import org.sicut.sqlite.Connector;
...
public final class Configurator {
    ...
    * @param propDb The properties sqlite database path.
    ...
    * @param propSql The sql source code that is used to create the properties database.
    */
    public void prepare(String prefDb, String propDb, String homeDir, InputStream prefSql, InputStream propSql) {
        ...
        db = new File(propDb);
        if ( !db.exists() || db.length() == 0 ) {
            con.setDatabase(propDb);
            createDB(propSql);
        }
    }
    
    public void prepare(String database, InputStream sql) {
